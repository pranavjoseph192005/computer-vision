...
def backproject(depth, intrinsic_matrix):
    ...
    pcloud = np.dstack((X, Y, Z))  # shape (H, W, 3)
...
if __name__ == '__main__':
    ...
    # rgb image
    rgb_filename = 'data/000008-color.jpg'
    im = cv2.imread(rgb_filename)
    
    # depth image
    depth_filename = 'data/000008-depth.png'
    depth = cv2.imread(depth_filename, cv2.IMREAD_ANYDEPTH)
    # convert from mm to m
    depth = depth / 1000.0
    
    # read the mask image
    mask_filename = 'data/000008-label-binary.png'
    ...
    # load matedata
    meta_filename = 'data/000008-meta.mat'
